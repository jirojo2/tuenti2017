const t = d.body,
    ...
    c = ["PMNR", "PIPYG", "QTNNG", "NRIC", "JCBX"];
let r = 300;
w.onclose = (() => setTimeout(() => l.reload(), 1e3));
let a = "";
for (let t = 0; t < e; t++) { a += "<tr>";
...
const m = t => {
    ...
    b = t => {
        ...
        const n = t.target;
        if ("TD" === n.nodeName)
            if (f) { $(f).classList.remove("cell-selected");
                const t = n.id,
                    e = `${f}-${t}`;
                w.send(btoa(`${e}-${m(e)}`)), f = "" } else f = n.id, n.classList.add("cell-selected") };
t.innerHTML = i, t.addEventListener("click", b);
let h = setInterval(() => {
    const t = $(`time${o}`);
    t ? (r--, r < 11 && (t.className = "big-time"), t.innerHTML = Math.max(0, r)) : clearInterval(h) }, 1e3);
